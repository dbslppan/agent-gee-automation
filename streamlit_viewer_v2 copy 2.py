...
# Sinkron dengan health_classes.py
CLASS_MAP = {
...
# Kolom tambahan dari shapefile yang ditampilkan di popup
# Sesuaikan dengan kolom yang ada di mumbul.gpkg
...
def render_map(gdf: gpd.GeoDataFrame, basemap: str, opacity: float):
    ...
    #     zoom_start=12,
    #     tiles=tile_url if tile_attr else tile_url,
    #     attr=tile_attr if tile_attr else None,
    # )

    m = folium.Map(location=[center_lat, center_lon], zoom_start=12, tiles=None)

    all_tiles = {
        "Satellite (Esri)": folium.TileLayer(
    ...
    # Tambahkan basemap yang dipilih user duluan (jadi aktif default)
    all_tiles[basemap].add_to(m)
    for name, tile in all_tiles.items():
        if name != basemap:
            tile.add_to(m)

    # Tambah OSM sebagai layer toggle
    # if tile_attr:
